...
/// @author Nazir Khan
/// @notice Handles multiple assets sent by a liquidity provider. Issues a shares representing entire deposited tokens amount
...
abstract contract Vault is ERC20, IVault {
    ...
    constructor(address[] memory _tokens, address[] memory _priceFeeds) ERC20("Vault Share", "vSHARE") {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            assets[token] = TokenInfo({token: IERC20(token), priceFeed: AggregatorV3Interface(feed), isActive: true});
            supportedAssets.add(token);
        }
    }

    function _latestPrice(AggregatorV3Interface feed) internal view returns (uint256, uint8) {
    ...
    function _tokenValueUsd(address asset, uint256 amount) internal view returns (uint256) {
        ...
        // return (amount * price * (10 ** targetDecimals)) / (10 ** (tokenDecimals + feedDecimals));

        // So:

        // 1 USDC (1e6) × $1 USDC (1e8) →
        // (1e6 * 1e8 * 1e18)/(10 ** 6(token decimals) * 8 (feed decimals))=1e18
        // $1 * 1e18 units. (scalled to 1e18)

        // 1 WETH (1e18) × $2000 (2 000 000 000 00) →
        // (1e18 * 2e11 * 1e18)/(10 ** (18 + 8)) = 2e21 → $2000 * 1e18 units.
    }

    // @ TODO : ADD REENTRANCY GUARD IS NEEDED OR NOT ?

    function _deposit(address _receiver, address[] memory _assets, uint256[] memory _amounts)
        internal
        returns (DepositDetails memory depositDetails)
    {
        ...
        if (_assets.length != _amounts.length) {
            revert VaultErrors.Vault__lengthMismatch();
        }

        uint256 vaultValueBefore = _totalVaultValueUsd();
        uint256 totalDepositValueUsd;
        (address[] memory filteredAssets, uint256[] memory filteredAmounts) = _filterSupportedTokens(_assets, _amounts);
        ...
        if (actualIncrease < (totalDepositValueUsd * VaultConstants.DEPOSIT_TOLERANCE_BPS) / 10000) {
            revert VaultErrors.Vault__DepositValueMismatch(totalDepositValueUsd, actualIncrease);
        }
        ...
        uint256 shares = _calculateSharesToMint(totalDepositValueUsd, vaultValueBefore);
        _mint(_receiver, shares);

        depositDetails = DepositDetails({sharesMinted: shares, totalUsdAmount: totalDepositValueUsd});

        // ✅ Emit event
        emit Deposited(_receiver, shares, filteredAssets, filteredAmounts, totalDepositValueUsd);
    }

    function _withdraw(address _owner, uint256 _shares, address _receiver)
        internal
        returns (WithdrawDetails memory withdrawDetails)
    {
        ...
        require(_shares > 0 && _shares <= balanceOf(owner), "Invalid/No shares");
        require(block.timestamp >= lastWithdrawal[owner] + VaultConstants.WITHDRAWAL_COOLDOWN, "Cooldown is active");
        // @audit
        address receiver = _receiver == address(0) ? msg.sender : _receiver;

        lastWithdrawal[owner] = block.timestamp;
        (address[] memory tokens, uint256[] memory amounts, uint256 withdrawValueUsd) = _calculateWithdraw(_shares);
        ...
        for (uint256 i = 0; i < n;) {
            ...
            if (amt > 0) {
                ...
                require(info.token.balanceOf(address(this)) >= amt, "Insufficient token balance");
        ...
        withdrawDetails = WithdrawDetails({
            tokensWithdrawn: tokens, amountsWithdrawn: amounts, withdrawValueUsd: withdrawValueUsd, sharesBurnt: _shares
        });
        // emit
    }

    function _filterSupportedTokens(address[] memory _assets, uint256[] memory _amounts)
        internal
        view
        returns (address[] memory, uint256[] memory)
    {
        ...
        uint256 maxTokens = VaultConstants.MAX_ALLOWED_TOKENS; // may be we can do something with the max length ...W
        ...
        for (uint256 i = 0; i < len;) {
            ...
            if (address(info.token) == address(0)) {
                revert VaultErrors.Vault__AssetNotSupported(asset);
            }
            if (!info.isActive) {
                revert VaultErrors.Vault__AssetNotActive(asset);
            }
            bool found;
    ...
    function _addAssets(address[] calldata _newAssets, address[] calldata _priceFeeds) internal {
        require(_newAssets.length == _priceFeeds.length, VaultErrors.Vault__lengthMismatch());
    ...
    function _addAsset(address _newAsset, address _priceFeed) internal {
        require(
            _newAsset != address(0) && _priceFeed != address(0), "Invalid Address for new asset or Price Feed address"
        );
        require(supportedAssets.length() < VaultConstants.MAX_ALLOWED_TOKENS, "MAX NUMBER OF TOKENS REACHED");

        require(address(assets[_newAsset].token) == address(0), "Asset already exists");
        ...
        assets[_newAsset] = TokenInfo({
            ...
            // add a decimal as well ?? PRICE FEED RETURNS DECIMALS AS WELL BUT CAN BE DIFFERENT !!
            isActive: true
        });
    ...
    function _removeAsset(address _asset) internal {
        ...
        require(address(assets[_asset].token) != address(0), "Asset Does not exist");

        // checks if the given asset has some amount before removing
        require(IERC20(_asset).balanceOf(address(this)) < 1e6, "Funds are still in the vault");
    ...
    function _updatePriceFeed(address _asset, address _newPriceFeed) internal {
        // checks if the given asset is a valid address
        require(address(assets[_asset].token) != address(0), "Invalid asset address");
        // verify pricefeed works
    ...
    function _calculateSharesToMint(uint256 _depositValueUsd, uint256 _vaultValueBefore)
        internal
        view
        returns (uint256 shares)
    {
        ...
        if (supply == 0) {
            ...
            shares = VaultConstants.INITIAL_SUPPLY; // or we can do shares = _depositValueUsd; (1 share = 1 USD value)
        } else {
    ...
    function _calculateDeposit(address[] calldata _assets, uint256[] calldata _amounts)
        internal
        view
        returns (uint256 shares)
    {
        ...
        require(len != 0 && len == _amounts.length, "Invalid length of assets and ammount");
     ...
     */
    function _calculateWithdraw(uint256 _shares)
        internal
        view
        returns (address[] memory assetsArr, uint256[] memory amountsArr, uint256 withdrawUsdValue)
    {
    ...
    function _lockShares(address _owner, uint256 _shares, uint64 _destChain) internal {
        // lock shares
        lockedShares[_destChain][_owner] += _shares;
        totalPendingShares += _shares;
    }

    function _unlockShares(address _owner, uint256 _shares, uint64 _destChain) internal {
        // unlock shares
        lockedShares[_destChain][_owner] -= _shares;
        totalPendingShares -= _shares;
    }
    ...
    function _shareValueUsd(uint256 shares) internal view returns (uint256) {
         ...
         * 100e18 = 100e36  ← WRONG! Should be 100e18 ($100) that's divided by 1e18 to prevent the overflow
         */
    }
     ...
     */
    function _getAllUserSharesValueUsd(address _user) internal view returns (uint256 totalAmtUsd) {
        ...
        for (uint16 i = 0; i < supportedAssets.length(); i++) {
            ...
            uint256 amountOut = (totalTokenInVault * sharesOfUser) / totalSharesMinted;
    ...
    function tokenValueUsd(address asset, uint256 amount) external view returns (uint256) {
        return _tokenValueUsd(asset, amount);
    }

    function totalTokensValueUsd(address[] calldata assets, uint256[] calldata amounts)
        external
        view
        returns (uint256 totalValue)
    {
        if (assets.length != amounts.length) revert VaultErrors.Vault__lengthMismatch();

        for (uint256 i = 0; i < assets.length;) {
            totalValue += _tokenValueUsd(assets[i], amounts[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function calculateSharesToMint(uint256 _depositValueUsd, uint256 _vaultValueBefore)
        external
        view
        returns (uint256 shares)
    {
    ...
    function calculateDeposit(address[] calldata _assets, uint256[] calldata _amounts)
        external
        view
        returns (uint256 shares)
    {
    ...
    function calculateWithdraw(uint256 _shares)
        external
        view
        returns (address[] memory assetsArr, uint256[] memory amountsArr, uint256 withdrawUsdValue)
    {
     ...
     */
    function getSupportedAssets() external view returns (address[] memory tokensAddresses) {
    ...
    function shareValueUsd(uint256 shares) external view override returns (uint256) {
